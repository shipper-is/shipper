...
4. Write a lightweight spike file named after the spike (kebab-case). This file is the single source of truth for the spike's status. If the user opted into a worktree, create it in the worktree's `.shipper/open/`; otherwise create it in the main checkout's `.shipper/open/`. Follow [./GIT.md](./GIT.md) for branch and worktree frontmatter. Format:
...
Use YAML frontmatter at the very top with `type: spike`, `branch`, `base_branch`, `started_at` (quoted ISO 8601 timestamp), and `worktree` when applicable — see GIT.md. No `## Phase` headings are needed — use a flat `- [ ]` task checklist only.
